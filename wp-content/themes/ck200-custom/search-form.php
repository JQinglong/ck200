...
if ($act == 1) {
	...
	// wpdbオブジェクト
	global $wpdb;
    $sql = $wpdb->prepare("SELECT MT.MUSIC_CD, MT.TITLE_NM FROM CK200_M_TITLE MT ORDER BY TITLE_NM" , '');
	// クエリ実行
	...
	if ($rows_title) {
	  echo "<select class=\"form-control\" name=\"search_data[music]\">";
		foreach ($rows_title as $row) {
		  echo "<option value=\"".$row->MUSIC_CD."\">".$row->TITLE_NM."</option>";
		}
	  echo "</select>";
	}
